...
mule-perf を 0.1 vCore に縮小しても、ロードジェネレータとしての精度が維持されることを確認。
...
mule-perf 0.1 vCore のシステム負荷: CPU 10%, Heap 118/486 MB — 十分余裕あり。

### 結論

- **Echo/Small DW は ±4% 以内で一致** — 1 vCore でも 0.1 vCore でも精度は同等
- **Heavy DW は 0.1 vCore で最大 -10%** — GC/スケジューリングのオーバーヘッドがテール分布に影響
- mule-perf は **0.1 vCore でも信頼できるロードジェネレータ** として使用可能
- 高精度が必要な場合は 1 vCore 推奨 (±3% 以内)
